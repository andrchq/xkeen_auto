...
echo ""
printf "Хотите настроить Telegram сейчас? (y/n): "
read -r SETUP_TG

if [ "$SETUP_TG" = "y" ] || [ "$SETUP_TG" = "Y" ]; then
    echo ""
    printf "Введите ID топика (получили у администратора @prsta_helpbot): "
    read -r TG_TOPIC_ID
    ...
    echo ""
    printf "Отправить тестовое уведомление? (y/n): "
    read -r TEST_TG
...
if [ -n "$SUBSCRIPTION_URL" ]; then
    ...
    if ./xkeen_sync.sh "$SUBSCRIPTION_URL"; then
        ...
        echo ""
        printf "Активировать первый доступный сервер? (y/n): "
...
echo "╚════════════════════════════════════════════════════════════╝"
echo ""
printf "Настроить автоматическую проверку доступности? (y/n): "
read -r SETUP_CRON
...
show_header
printf "Настроить автоматический выбор сервера после перезагрузки роутера? (y/n): "
...
echo ""
printf "Удалить установочный скрипт? (y/n): "
