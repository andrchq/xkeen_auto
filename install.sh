...
is_technical_server() {
    CC="$1"
    # RUSSIA/Россия не актуальна для работы на роутере - исключаем
    ...
    # RUSSIA|RU исключена - не актуальна для работы на роутере
...
printf "Спасибо за доверие к сервису ${BLUE}${BOLD}простовпн${RESET}.\n"
printf "Мы сделали установку максимально понятной: никакой магии —\n"
printf "только технологии, созданные для людей.\n\n"

printf "В ближайшие минуты мы:\n"
printf "• Подготовим роутер и окружение.\n"
printf "• Установим модули и автоматизацию.\n"
printf "• Подключим подписку и проверим работу.\n\n"

printf "${GREEN}💬 Поддержка:${RESET} https://t.me/prsta_helpbot\n"
printf "${BLUE}🤖 Бот для покупок и продлений:${RESET} https://t.me/prstabot\n"
printf "${ORANGE}${LINE}${RESET}\n\n"


log "Начинаю установку..."
pause_for_reading "$TIMER_START"
...
show_header
show_section "Подготавливаем систему"
printf "Проверяем, хватает ли на роутере необходимых инструментов.\n"
printf "Если чего-то нет — установим автоматически, без лишних вопросов.\n\n"
...
show_header
show_section "Готовим рабочие папки"
printf "Создаём аккуратную структуру, где будут храниться конфигурации,\n"
printf "резервные копии и вспомогательные файлы сервиса.\n\n"
...
show_header
show_section "Загружаем модули Простовпн"
printf "Берём свежие версии скриптов из репозитория и размещаем их на роутере.\n"
printf "Так вы получаете все последние улучшения и исправления.\n\n"
...
log "✓ Основные модули на месте"
pause_for_reading "$TIMER_SCRIPTS_LOADED"

show_header
show_section "Включаем автозапуск"
printf "Добавляем сервисы, которые будут следить за VPN и запускаться\n"
printf "при каждой перезагрузке роутера.\n\n"
...
pause_for_reading "$TIMER_INIT_SCRIPTS"

show_header
show_section "Закрепляем права доступа"
printf "Даем скриптам нужные разрешения, чтобы они запускались без препятствий.\n\n"
...
log "✓ Права закреплены"
pause_for_reading "$TIMER_PERMISSIONS"

show_header
show_section "Команда prosto готова"
printf "Добавляем фирменную команду, с которой вы будете управлять\n"
printf "ротацией серверов и подписками в пару кликов.\n\n"
...
if command -v xkeen >/dev/null 2>&1; then
    ...
    CONFIGS_INSTALLED=1
    pause_for_reading "$TIMER_XRAY_CONFIGS"
fi
...
printf "Проверяем, доходят ли уведомления. Это занимает пару секунд,\n"
printf "и вы сразу увидите сообщение у себя в Telegram.\n\n"
...
printf "${ORANGE}${LINE}${RESET}\n"
pause_for_reading "$TIMER_TELEGRAM_TEST"

# 5. Обязательный ввод URL подписки
SUBSCRIPTION_FILE="$INSTALL_DIR/.subscription_url"
show_header
show_section "Настройка подписки"
printf "Укажите ссылку на подписку: мы загрузим список серверов и будем\n"
printf "держать его в актуальном состоянии автоматически.\n\n"
...
if sh ./xkeen_sync.sh "$SUBSCRIPTION_URL"; then
    ...
    pause_for_reading "$TIMER_SERVERS_LIST"
...
⏰ $TIMESTAMP"
        ...
        pause_for_reading "$TIMER_SERVER_ACTIVATE"
    else
    ...
    if [ "$CONFIGS_INSTALLED" -eq 1 ] && [ -f "$CONFIG_DIR/configs/04_outbounds.json" ]; then
        ...
        pause_for_reading "$TIMER_XRAY_RESTART"
    fi
else
...
show_header
show_section "Автоматические проверки подключены"
printf "Будем регулярно проверять доступность серверов и при необходимости\n"
printf "менять их без вашего участия.\n\n"
...
log "✓ Автоматическая проверка включена (каждые 2 минуты)"
log "✓ Автозапуск сервиса активирован"
pause_for_reading "$TIMER_CRON_SETUP"

# 10. Система мониторинга - ДА
show_header
show_section "Система мониторинга"
printf "Если интернет на роутере вдруг станет нестабилен, сервис сам\n"
printf "попробует всё восстановить и сообщит вам об этом.\n\n"
...
log "✓ Мониторинг сети включён (каждые 5 минут)"
log "✓ Система готова поддерживать стабильность автоматически"
pause_for_reading "$TIMER_MONITORING_SETUP"

# ============ Открытие портов ============
show_header
show_section "Настраиваем порты"
printf "Открываем рекомендованные порты, чтобы сервис мог работать\n"
printf "стабильно и принимать соединения без ограничений.\n\n"
...
if command -v xkeen >/dev/null 2>&1; then
    ...
    pause_for_reading "$TIMER_PORTS_OPEN"
else
...
printf "• Покупка и продление: ${BLUE}https://t.me/prstabot${RESET}\n"
printf "Хорошего и безопасного интернета!\n\n"
